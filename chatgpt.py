...
def navigate_to_related_page():
    links = browser.find_elements(By.CSS_SELECTOR, "div.mw-parser-output a")  # Поиск всех ссылок в содержимом статьи
    ...
    print("\nСвязанные страницы:")
    for i, link in enumerate(related_links[:10], 1):  # Отображаем только первые 10 ссылок
    ...
    while True:
        try:
            ...
            if link_choice == 0:
                return
            elif 1 <= link_choice <= len(related_links[:10]):
                chosen_link = related_links[link_choice - 1].get_attribute("href")
